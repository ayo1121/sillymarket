        ...
        <div ref={wrapperRef} className={cn("relative", className)}>
            <div className="relative">
                ...
                <input
                    ...
                    className="w-full win95-sunken bg-background font-bold pl-10 pr-4 py-2 text-base focus:outline-none focus:ring-2 focus:ring-primary/50 transition-all"
                />
                {value.length > 0 && (
                    <button
                        onClick={() => onChange('')}
            ...
            {isOpen && suggestions.length > 0 && (
                <div className="absolute z-50 w-full mt-1 bg-background border-2 border-foreground shadow-[4px_4px_0px_0px_rgba(0,0,0,1)]">
                        ...
                        {suggestions.map((market, index) => (
                            <li
                                ...
                                className={cn(
                                    ...
                                    index === focusedIndex ? "bg-primary/10" : "hover:bg-primary/5"
                                )}
                                onClick={() => handleSelect(market)}
                                onMouseEnter={() => setFocusedIndex(index)}
                            >
                                <div className="flex flex-col min-w-0">
                                    <span className="font-bold truncate text-sm">{market.displayQuestion}</span>
